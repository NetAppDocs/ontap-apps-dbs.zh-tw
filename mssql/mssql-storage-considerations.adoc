...
聚合是 AFF 儲存配置的最低層級儲存容器。網路上一些舊文件建議將 I/O 分離到不同的底層磁碟機集。不建議在 ONTAP 中採用此做法。NetApp 使用共用和專用聚合，並將資料檔案和交易日誌檔案分開，進行了各種 I/O 工作負載特徵測試。測試顯示，一個包含更多 RAID 群組和磁碟機的大型聚合可以最佳化並提升儲存效能，且基於以下兩個原因，管理員也更易於管理：

* 一個大型聚合/SA 使所有磁碟機的 I/O 功能可供所有檔案使用。
