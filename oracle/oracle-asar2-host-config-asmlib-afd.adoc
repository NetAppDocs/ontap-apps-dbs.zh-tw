...
summary: 在ASA r2 上使用 Linux 和 ASMlib/ASM 過濾器驅動程式的 Oracle 資料庫 
searchtitle: 在ASA r2 上使用 ASMLib/AFD（ASM 過濾器驅動程式）的 Oracle 資料庫 
---
...
[role="lead"]
使用 AFD 和 ASMlib 的ASA r2 ONTAP的 Linux 作業系統特有的設定主題。



== ASMLib 區塊大小

ASMlib 是一個可選的 ASM 管理函式庫和相關實用程式。它的主要價值在於能夠將 LUN 標記為 ASM 資源，並帶有人類可讀的標籤。
