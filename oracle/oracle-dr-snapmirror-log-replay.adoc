...
最重要的要求是將資料檔案隔離到一個或多個專用容器中。這可以透過多種方式實現，包括將所有資料檔案放置在單一 LUN/命名空間、單一 AFF NFS 磁碟區中，或放置在多磁碟區或多 LUN/命名空間的 ONTAP 一致性群組中。關鍵在於資料檔案必須不受任何其他檔案類型的污染。這樣做的目的是確保資料檔案複寫完全獨立於其他資料類型（例如歸檔日誌）的複寫。有關檔案佈局的更多資訊以及確保儲存佈局支援快照的重要細節，請參閱 link:oracle-dp-online-backup.html#data-layout["資料配置"]。
