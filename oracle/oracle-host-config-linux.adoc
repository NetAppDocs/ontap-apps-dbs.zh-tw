...
當 (a) 啟用 DNFS 且 (b) 來源 NFS 共用在單一伺服器上多次掛載 (c) 存在嵌套 NFS 掛載時，需要一個名為 `nosharecache`的額外掛載選項。這種配置主要見於支援 SAP 應用程式的環境。例如，NetApp 系統上的單一 NFS 共用可能包含兩個目錄：一個位於 `/vol/oracle/base`，另一個位於 `/vol/oracle/home`。如果 `/vol/oracle/base`掛載在 `/oracle`，而 `/vol/oracle/home`掛載在 `/oracle/home`，則會產生源自相同來源的巢狀 NFS 掛載。

作業系統可以偵測到 `/oracle`和 `/oracle/home`位於同一個 NFS 共用上，也就是同一個來源檔案系統。然後，作業系統會使用同一個裝置句柄來存取資料。這樣做可以提高作業系統快取和其他一些操作的效率，但會幹擾 DNFS。如果 DNFS 必須存取檔案（例如 `spfile`）位於 `/oracle/home`上，它可能會錯誤地嘗試使用錯誤的資料路徑。這會導致 I/O 操作失敗。在這些組態中，對於與主機上另一個 NFS 檔案系統共用來源 NFS 共用的任何 NFS 檔案系統，請新增 `nosharecache`掛載選項。這樣做會強制 Linux 作業系統為該檔案系統指派一個獨立的裝置句柄。
