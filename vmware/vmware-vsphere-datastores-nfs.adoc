...
* 在 ONTAP 叢集的每個節點上，每個 SVM 都應使用單一邏輯介面（LIF）。先前建議每個資料儲存使用一個 LIF 的做法已不再必要。雖然直接存取（LIF 和資料儲存位於同一節點上）效果最佳，但不必擔心間接存取，因為其效能影響通常微乎其微（僅幾微秒）。
...
[TIP]
====
* nconnect 在 vSphere 8 的最大值為 4，預設值為 1。vSphere 中的最大值限制可以透過進階設定在每個主機上提高，雖然通常不需要。
* 對於需要比單一 TCP 連線更高效能的環境，建議使用值 4 。
* 請注意、ESXi 的 NFS 連線數上限為 256 個、每個 nconnect 連線都會計入這個總數。例如、兩個資料存放區的 nconnect=4 會計為總共 8 個連線。
* 在生產環境中實施大規模變更之前，請務必測試 nconnect 對您環境的效能影響。
...
* 如 link:vmware-vsphere-settings.html["設定"] 所述，如果您未使用 vSphere CSI for Kubernetes，則應依照  https://knowledge.broadcom.com/external/article/386364/reducing-excessive-vsan-cnssync-warnings.html["VMware知識庫386364"^] 設定 newSyncInterval
...
* 儘管 ONTAP 提供了一種靈活的磁碟區命名空間結構，允許使用連接點將磁碟區排列成樹狀，但這種方法對於 vSphere 並無價值。它會在資料儲存區的根目錄中為每個 VM 建立一個目錄，而不考慮儲存設備的命名空間階層。因此，最佳實務做法是將 vSphere 磁碟區的連接點路徑直接掛載到 SVM 的根磁碟區上，這也是 ONTAP tools for VMware vSphere 配置資料儲存區的方式。沒有巢狀連接點路徑也意味著除了根磁碟區之外，任何磁碟區都不依賴其他磁碟區，並且即使是刻意將磁碟區離線或銷毀，也不會影響到其他磁碟區的路徑。
