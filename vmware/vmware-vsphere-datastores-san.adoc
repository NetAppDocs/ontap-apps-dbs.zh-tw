...
vSphere 內建支援多條通往儲存設備的路徑。vSphere 可偵測受支援儲存系統的儲存裝置類型，並自動配置多路徑堆疊以支援正在使用的儲存系統的功能，無論使用何種協議，或是否使用 ASA、AFF、FAS 或軟體定義的 ONTAP。
...
NOTE: 請參閱 link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["VMware 組態最大值工具"^] 以瞭解 ESXi 中支援的最新限制。
...
* 對於 iSCSI 或 NVMe/TCP 網路，當存在多個虛擬交換器時，應在不同的未路由網路子網路上使用多個 VMkernel 網路介面，並啟用 NIC 群組。您還可以使用連接到多個實體交換器的多個實體 NIC 來提供 HA 並提高處理量。在 ONTAP 中，設定單一模式介面群組以進行容錯移轉，其中包含連接到兩個或多個交換器的兩個或多個連結，或使用 LACP 或其他連結彙總技術搭配多模式介面群組，以提供 HA 和連結彙總的優勢。請注意，必須在每個 VMkernel 連接埠和每個實體介面上啟用 NVMe/TCP，而且您必須對齊它們，以便 NVMe/TCP VMkernel 連接埠能夠存取已啟用 NVMe/TCP 的實體 NIC。下圖說明使用 IP 型 SAN 傳輸協定的多重路徑連線。image:vsphere_ontap_image2.png["多重路徑連線"]
